...
function coercePreferredTerms(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const entries = Object.entries(raw)
        .map(([key, value]) => {
        if (typeof value === 'string' && value.trim()) {
            return [String(key).trim(), value.trim()];
        }
        return null;
    })
...
function normalizeIndicatorLabel(raw) {
    let label = null;
    let touched = false;
    if (raw.preferred_terms && typeof raw.preferred_terms === 'object') {
        const preferred = raw.preferred_terms;
...
function extractIndicatorChoices(raw) {
    let touched = false;
    const choices = [];
...
function coerceProfileString(raw, keys) {
...
function sanitizeCustomCriteria(raw) {
...
function sanitizeSignalPreferences(raw) {
    if (!Array.isArray(raw))
        return [];
    const rows = [];
    for (const item of raw) {
...
function sanitizeDisqualifiers(raw) {
    if (!Array.isArray(raw))
        return [];
    const rows = [];
    for (const item of raw) {
...
function sanitizePromptConfig(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
...
export function extractSaveProfilePayloads(raw, limit = 4) {
    if (typeof raw !== 'string' || !raw.includes('save_profile'))
        return [];
    const payloads = [];
    const seen = new Set();
...
async function applySinglePayload(supabase, userId, payload) {
    const summary = [];
    let profileRow = null;
    let customCriteria = [];
    let signalPreferences = [];
    let disqualifiers = [];
    ...
    if (payload.profile && typeof payload.profile === 'object') {
        ...
        const updatePayload = { ...sanitized.values };
        if (existingProfile) {
            if (Array.isArray(updatePayload.indicator_choices)) {
                if (updatePayload.indicator_choices.length === 0) {
                    ...
                    const existing = Array.isArray(existingProfile.indicator_choices)
                        ? existingProfile.indicator_choices.map((entry) => typeof entry === 'string' ? entry.trim() : '').filter(Boolean)
                        : [];
...
export async function applySaveProfilePayloads(supabase, userId, payloads) {
    const aggregateSummary = [];
    let finalProfile = null;
    let finalCriteria = [];
    let finalSignals = [];
    let finalDisqualifiers = [];
    ...
    for (const payload of payloads) {
        ...
        if (payload.custom_criteria !== undefined)
            finalCriteria = result.customCriteria;
        if (payload.signal_preferences !== undefined)
            finalSignals = result.signalPreferences;
        if (payload.disqualifying_criteria !== undefined)
            finalDisqualifiers = result.disqualifyingCriteria;
        if (payload.prompt_config !== undefined)
            finalPromptConfig = result.promptConfig;
    }
    return {
