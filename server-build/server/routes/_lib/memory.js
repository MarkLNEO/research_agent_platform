...
export function buildMemoryBlockFromData(agent, knowledgeRows = [], implicitRows = [], maxBytes = DEFAULT_MEMORY_MAX_BYTES) {
    ...
    const tendencies = implicitRows
        ...
        .filter(Boolean)
        .slice(0, 12);
    if (confirmed.length === 0 && tendencies.length === 0)
        return '';
    let block = `<<memory v=1 agent=${agent}>`;
    const ensureFits = (candidate) => Buffer.byteLength(candidate, 'utf8') <= maxBytes;
...
export async function buildMemoryBlock(userId, agent = 'company_research') {
    if (!serviceClient)
        return '';
    try {
...
export async function recordPreferenceSignal(userId, agent, key, observed, weight = 1) {
    if (!serviceClient)
        return null;
    const client = getServiceClient();
    const payload = {
