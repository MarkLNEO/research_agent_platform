...
function formatConfidence(value) {
    ...
    if (typeof value.conf === 'number')
        return value.conf;
    return 0;
}
function inferLengthChoice(value) {
    ...
    if (typeof value.scale01 === 'number') {
        if (value.scale01 <= 0.34)
            return 'brief';
        if (value.scale01 >= 0.66)
            return 'long';
        return 'standard';
    }
    return null;
}
function buildSuggestion(key, value) {
    ...
    switch (key) {
        ...
        case 'tone': {
            const scalar = typeof value.value === 'number'
                ? value.value
...
export default async function handler(req, res) {
    ...
    try {
        ...
        for (const row of implicitRows) {
            ...
            if (!pairMap.has(key))
                pairMap.set(key, { userId: row.user_id, agent: row.agent });
        }
        const knowledgeCache = new Map();
        await Promise.all(Array.from(pairMap.values()).map(async ({ userId, agent }) => {
        ...
        for (const row of implicitRows) {
            ...
            if (confidence < MIN_CONF_FOR_SUGGESTION)
                continue;
            ...
            if (!cacheEntry)
                continue;
            const alreadyExplicit = cacheEntry.knowledge.some(entry => entry.content.trim().toLowerCase().includes(suggestion.content.trim().toLowerCase()));
            if (alreadyExplicit)
                continue;
