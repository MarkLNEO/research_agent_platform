...
export async function ensureTable(table) {
    const key = table === 'entity_aliases' || table === 'user_entity_aliases' ? 'entity_alias_bundle' : table;
    if (!ensurePromises.has(key)) {
        ensurePromises.set(key, (async () => {
            const config = getConfig();
            const tablesToCheck = key === 'entity_alias_bundle'
