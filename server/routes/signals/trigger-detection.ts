...
type Pref = { signal_type: string; importance: 'critical'|'important'|'nice_to_have'; lookback_days: number; config?: any };

async function detectForAccount(account: any, prefs: Pref[], openaiKey: string) {
  ...
  const normalized = arr.map((raw: any) => normalizeSignal(raw, uniqueTypes));
  return normalized.filter(Boolean);
}

function extractOutputText(apiResponse: any): string {
  if (Array.isArray(apiResponse?.output)) {
    const message = apiResponse.output.find((i: any) => i?.type === 'message');
    if (message?.content?.[0]?.text) return String(message.content[0].text);
    const textOutput = apiResponse.output.filter((i: any) => i?.type === 'output_text' && typeof i.text === 'string').map((i: any) => i.text).join('\n');
    if (textOutput) return textOutput;
  }
  if (apiResponse?.content) return String(apiResponse.content);
  if (apiResponse?.output_text) return String(apiResponse.output_text);
  return '';
}
...
function parseJsonArrayFromText(text: string): any[] {
  ...
  try { const parsed = JSON.parse(jsonText); return Array.isArray(parsed) ? parsed : (Array.isArray(parsed?.signals) ? parsed.signals : []); } catch { return []; }
}

function norm(v: string) { return String(v || '').toLowerCase().replace(/\s+/g, '_').replace(/-+/g, '_').trim(); }
function dedupeBy<T>(arr: T[], key: (t: T)=>string) { const m = new Map<string,T>(); for (const x of arr) { const k = key(x); if (!m.has(k)) m.set(k, x); } return [...m.values()]; }

function buildHints(prefs: Pref[]) {
  const types = prefs.map((p) => norm(p.signal_type));
  const focus: string[] = [];
  if (types.some((t) => t.includes('breach') || t.includes('security'))) focus.push('security breaches/incidents/ransomware');
  if (types.some((t) => t.includes('leadership'))) focus.push('leadership changes (CISO/CTO/CEO)');
  if (types.some((t) => t.includes('funding'))) focus.push('funding rounds');
  if (types.some((t) => t.includes('hiring'))) focus.push('hiring surges');
  return { focus: focus.join(', ') || 'material company events' };
}

function normalizeSignal(raw: any, prefs: Pref[]) {
  ...
  if (!description || !source_url || !source_url.startsWith('http')) return null;
  const confidenceNorm = String(raw.confidence || 'medium').toLowerCase();
  const confidence: 'low'|'medium'|'high' = confidenceNorm.includes('high') ? 'high' : confidenceNorm.includes('low') ? 'low' : 'medium';
  ...
  const severity: 'critical'|'high'|'medium'|'low' = score >= 90 ? 'critical' : score >= 75 ? 'high' : score >= 55 ? 'medium' : 'low';
