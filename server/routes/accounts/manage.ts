  ...
  try {
    ...
    switch (action) {
      ...
      case 'list': {
        ...
        else if (body.filter === 'stale') query = query.or(`last_researched_at.lt.${new Date(Date.now() - 14*24*60*60*1000).toISOString()},last_researched_at.is.null`);

        const { data, error } = await query;
        if (error) throw error;
        const accounts = (data || []).map((a: any) => {
          const signals = a.recent_signals || [];
          return { ...a, signal_count: signals.length, unviewed_signal_count: signals.filter((s: any) => !s.viewed).length };
        });
        const stats = {
          ...
          with_signals: accounts.filter((a: any) => (a.recent_signals || []).length > 0).length,
          stale: accounts.filter((a: any) => { const d=a.last_researched_at?new Date(a.last_researched_at).getTime():0; return !d || (Date.now()-d)>(14*24*60*60*1000); }).length,
        };
