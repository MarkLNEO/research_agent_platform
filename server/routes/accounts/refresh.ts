...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!accounts || accounts.length === 0) return res.status(200).json({ message: 'No accounts need refreshing', processed: 0 });
    ...
    for (const account of accounts) {
      try {
        ...
        if (Array.isArray(research.signals) && research.signals.length) {
          const rows = research.signals.map((s: any) => ({
...
async function quickResearch(companyName: string, key: string) {
  const body = {
    model: 'gpt-5-mini',
    input: [
  ...
  if (!jsonText && Array.isArray(data.output)) {
    const textPart = data.output.find((p: any) => p.type === 'output_text');
