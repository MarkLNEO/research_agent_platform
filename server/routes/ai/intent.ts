...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    for await (const chunk of stream as any) {
      if (chunk.type === 'response.output_text.delta' && chunk.delta) buf += chunk.delta;
    }
    ...
    try {
      ...
      if (s >= 0 && e > s) parsed = JSON.parse(buf.slice(s, e + 1));
    } catch {}
    if (!parsed) parsed = { intent: 'other', subject: '', confidence_intent: 0.2, confidence_subject: 0 };
