...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (loadErr || !job) return res.status(404).json({ error: 'job not found' });
    ...
    async function processTask(task: any) {
      ...
      const { data: claimRows } = await supabase
        ...
        .eq('status', 'pending')
        .select('id');
      if (!Array.isArray(claimRows) || claimRows.length === 0) return;
...
async function runCompanyResearch(company: string, depth: 'quick'|'deep', key: string): Promise<string> {
  ...
  const text = Array.isArray(j.output)
    ? (j.output.find((p: any) => p.type === 'output_text')?.text || j.output.find((p: any) => p.type === 'message')?.content?.[0]?.text || '')
