...
export async function completeOnboardingFor(email: string) {
  const admin = await getAdmin();
  const userId = await getTestUserId(email);
  ...
  const profilePayload = {
    user_id: userId,
    company_name: existingProfile?.company_name ?? 'Nevereverordinary Labs',
    company_url: existingProfile?.company_url ?? 'https://nevereverordinary.com',
    linkedin_url: existingProfile?.linkedin_url ?? 'https://www.linkedin.com/company/nevereverordinary',
    youtube_channel: existingProfile?.youtube_channel ?? null,
    industry: existingProfile?.industry ?? 'Aerospace & Defense',
    user_role: existingProfile?.user_role ?? 'Account Executive',
    use_case: existingProfile?.use_case ?? 'Research strategic accounts',
    icp_definition: existingProfile?.icp_definition ?? 'Aerospace enterprises with 1000+ employees',
    target_titles: existingProfile?.target_titles ?? ['CISO', 'VP Security', 'Director of Security'],
    competitors: existingProfile?.competitors ?? ['Lockheed Martin', 'Raytheon'],
    research_focus: existingProfile?.research_focus ?? ['executive_summary', 'buying_signals', 'decision_makers'],
    additional_sources: existingProfile?.additional_sources ?? ['https://nevereverordinary.com/blog'],
    onboarding_complete: true,
    onboarding_step: 9,
    onboarding_data: existingProfile?.onboarding_data ?? {},
    metadata: existingProfile?.metadata ?? {},
    output_format: existingProfile?.output_format ?? 'pdf',
    output_style: existingProfile?.output_style ?? 'executive_summary',
    seniority_levels: existingProfile?.seniority_levels ?? ['Executive', 'VP'],
    target_departments: existingProfile?.target_departments ?? ['Security', 'IT'],
    default_report_sections: existingProfile?.default_report_sections ?? [
      'executive_summary',
      'buying_signals',
      'custom_criteria',
      'decision_makers',
  ...
  await admin.from('company_profiles').upsert(profilePayload, { onConflict: 'user_id' });
  await admin.from('users').upsert({
    id: userId,
    credits_remaining: 1000,
    credits_total_used: 0,
...
async function waitForWorkspaceReady(page: Page) {
  // Ensure HomeGate loader is bypassed across the session
  try { await page.evaluate(() => window.localStorage?.setItem('skipHomeGate', '1')); } catch {}
  const messageInput = page.getByLabel(/Message agent/i);
  const chatSurface = page.getByTestId('chat-surface');
  const greeting = page.getByTestId('dashboard-greeting');
  const sidebar = page.getByTestId('sidebar');

  const isReady = async (timeout: number) => {
    if (await messageInput.isVisible({ timeout }).catch(() => false)) return true;
    if (await chatSurface.isVisible({ timeout }).catch(() => false)) return true;
    if (await greeting.isVisible({ timeout }).catch(() => false)) return true;
    if (await sidebar.isVisible({ timeout }).catch(() => false)) return true;
    return false;
  };

  if (await isReady(2500)) return;

  // Try to bypass onboarding quickly if present
  const onboarding = page.getByTestId('onboarding-welcome');
  if (await onboarding.isVisible({ timeout: 1500 }).catch(() => false)) {
    const immediate = page.getByTestId('onboarding-immediate');
    if (await immediate.isVisible({ timeout: 1500 }).catch(() => false)) {
      await immediate.click({ timeout: 3000 }).catch(() => {});
    } else {
      const suggestions = ['Research Boeing', 'Find companies like Stripe', 'What can you do?'];
      for (const text of suggestions) {
        const chip = page.getByRole('button', { name: text });
        if (await chip.isVisible({ timeout: 300 }).catch(() => false)) {
    ...
    if (await isReady(3000)) return;
  }

  // Handle HomeGate loader more robustly
  const loader = page.getByText('Preparing your workspace', { exact: false });
  const openDashboard = page.getByRole('button', { name: /Open last dashboard/i });
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    if (await isReady(1000)) return;
    // If redirected to login at any point, authenticate inline
    const atLogin = await page.getByRole('heading', { name: 'Sign In' }).isVisible({ timeout: 300 }).catch(() => false);
    if (atLogin) {
      ...
      try {
        ...
        await emailField.fill(email, { timeout: 2000 }).catch(() => {});
        await passwordField.fill(password, { timeout: 2000 }).catch(() => {});
        try { await page.keyboard.press('Enter'); } catch { await page.getByRole('button', { name: /Sign In/i }).click().catch(() => {}); }
    ...
    if (loaderVisible) {
      if (await openDashboard.isVisible({ timeout: 500 }).catch(() => false)) {
        await openDashboard.click().catch(() => {});
        await page.waitForTimeout(600);
        if (await isReady(1500)) return;
      }
      // Navigate to a page that doesn't rely on profile load
      await page.goto('/research').catch(() => {});
      const historyVisible = await page.getByTestId('research-history').isVisible({ timeout: 2000 }).catch(() => false);
      if (historyVisible) {
        await page.goto('/').catch(() => {});
        await page.waitForTimeout(500);
        if (await isReady(2000)) return;
      }
    } else {
  ...
  if (!(await isReady(2000))) {
    throw new Error('Workspace did not become ready after login');
  }
}
...
export async function login(page: Page, {
  ...
  const isSignedIn = async (timeout = 500) => {
    ...
    if (await page.getByText('Preparing your workspace', { exact: false }).isVisible({ timeout }).catch(() => false)) return true;
    if (await page.getByTestId('sidebar').isVisible({ timeout }).catch(() => false)) return true;
    return false;
  }
...
export async function ensureLoggedIn(page: Page) {
  ...
  if (await page.getByRole('heading', { name: 'Sign In' }).isVisible({ timeout: 1000 }).catch(() => false)) {
    await login(page, { email, completeOnboarding: true });
    return;
  }
  await completeOnboardingFor(email);
  // Bypass HomeGate loader explicitly for tests
  try { await page.evaluate(() => window.localStorage?.setItem('skipHomeGate', '1')); } catch {}
  await page.goto('/');
  await waitForWorkspaceReady(page);
}
...
export async function seedBulkResearchJob(opts?: {
  companies?: string[];
  researchType?: 'quick' | 'deep';
  status?: 'pending' | 'running' | 'completed' | 'failed';
  completedCount?: number;
  results?: Array<{ company: string; status: 'completed'|'failed'; result?: string; error?: string; completed_at: string }>;
}) {
  const admin = await adminClient();
  const email = process.env.E2E_EMAIL || 'codex.e2e@nevereverordinary.com';
  const userId = await getTestUserId(email);

  const companies = opts?.companies || ['Alpha Seed Co', 'Beta Seed LLC'];
  const researchType = opts?.researchType || 'quick';
  const status = opts?.status || 'pending';
  const completedCount = typeof opts?.completedCount === 'number' ? opts!.completedCount : (status === 'completed' ? companies.length : 0);
  ...
  if (opts?.results) payload.results = opts.results;

  const { error } = await admin.from('bulk_research_jobs').insert(payload);
  if (error) throw error;

  // Seed tasks minimally (optional, helpful if the UI expands later)
  try {
    const tasks = companies.map((c) => ({ job_id: id, user_id: userId, company: c, status: status === 'completed' ? 'completed' : 'pending', created_at: now }));
    await admin.from('bulk_research_tasks').insert(tasks);
  } catch {}

  return { jobId: id, userId };
}

export async function clearBulkJobsForUser(email = process.env.E2E_EMAIL || 'codex.e2e@nevereverordinary.com') {
  const admin = await adminClient();
  const userId = await getTestUserId(email);
  const { data: jobs } = await admin.from('bulk_research_jobs').select('id').eq('user_id', userId);
  const ids = (jobs || []).map((j: any) => j.id);
  if (ids.length > 0) {
    await admin.from('bulk_research_tasks').delete().in('job_id', ids);
    await admin.from('bulk_research_jobs').delete().in('id', ids);
  }
}
