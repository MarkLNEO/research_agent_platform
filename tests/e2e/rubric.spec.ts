...
async function captureOnboardingFlow(page: Page, email: string) {
  ...
  await login(page, { email, completeOnboarding: false });
...
async function captureSignalsAssets(page: Page) {
  ...
  if (reviewVisible) {
    ...
    // Fallback to sidebar account list; if unavailable, do not fail the entire run
    ...
    if (visible) {
      ...
      // Try to refresh signals context once
      await page.goto('/signals').catch(() => {});
      await page.getByTestId('all-signals').isVisible({ timeout: 5000 }).catch(() => {});
      await page.goto('/').catch(() => {});
      const reviewAgain = await reviewButton.isVisible({ timeout: 5000 }).catch(() => false);
      if (reviewAgain) {
        await reviewButton.click({ force: true });
      } else {
        // No drawer path available; capture only the feed and bail gracefully
        return;
      }
    }
  }
...
async function captureAccountAssets(page: Page) {
  await page.goto('/');
  const sidebar = page.getByTestId('sidebar');
  await captureElement(sidebar, 'account-dashboard');

  const hotCard = page.locator('[data-testid="account-list-item"][data-priority="hot"]').first();
  if (await hotCard.isVisible({ timeout: 5000 }).catch(() => false)) {
