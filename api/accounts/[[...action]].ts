...
import manage from '../../server-build/server/routes/accounts/manage.js';
import refresh from '../../server-build/server/routes/accounts/refresh.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const slug = normalize(req.query.action) || fromPath(req.url, '/api/accounts');
  const fn = handlers[slug];
  if (!fn) {
    res.status(404).json({ error: `No accounts handler for ${slug || '(root)'}` });
    return;
  }
